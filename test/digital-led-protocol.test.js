/**
 * @fileoverview Digital LED Protocol Tests
 * 
...
// Digital LED Protocol Tests

// P5-001: Digital board should show color warning for red
it('P5-001: should show color warning and turn LED on for red', async () => {
...
// P5-002: Digital board should show color warning for green
it('P5-002: should show color warning and turn LED on for green', async () => {
  const { LedController } = await import('../src/controller.js');
  ...
  // Check for warning log
  const hasWarning = capturedLogs.some(log => 
    log.includes('Digital LED does not support colors') && log.includes('green')
  );
  expect(hasWarning).toBe(true);
...
// P5-003: Digital board should show color warning for blue
it('P5-003: should show color warning and turn LED on for blue', async () => {
  const { LedController } = await import('../src/controller.js');
  ...
  // Check for warning log
  const hasWarning = capturedLogs.some(log => 
    log.includes('Digital LED does not support colors') && log.includes('blue')
  );
  expect(hasWarning).toBe(true);
...
// P5-004: Digital board should handle white color exception
it('P5-004: should handle white color as ON without warning', async () => {
  const { LedController } = await import('../src/controller.js');
  ...
  // White should not show warning for digital LEDs
  const hasWarning = capturedLogs.some(log => 
